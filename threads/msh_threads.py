...
#--------------------------------------Поток генерации сетки------------------------------------------#

class msh_generation_thread(QtCore.QThread):
	msh_start_sig = pyqtSignal('QString', 'QString', 'PyQt_PyObject', 'QString', 'QString')
	msh_fin_sig = pyqtSignal(int, 'QString', 'QString', 'PyQt_PyObject', 'QString', 'QString')
	...
	def run(self):
		...
		if os.path.exists(MeshDict_file) == True:
			...
			self.msh_start_sig.emit(prj_path_val_th, mesh_name_txt_val_th_a, par, int_lng, msh_t)
			...
			self.msh_fin_sig.emit(return_code, prj_path_val_th, mesh_name_txt_val_th_a, par, int_lng, msh_t)
