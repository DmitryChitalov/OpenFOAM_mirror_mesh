...
class file_0_form_class(QWidget):
        ...
        if self.con.open():
            if 'patches' in self.con.tables():
                ...
                for el_p in self.patches_lst:
                    ...
                    self.type_list = ["symmetryPlane", "tractionDisplacement", "empty", "zeroGradient", "noSlip", \
                        "calculated", "nutLowReWallFunction", "fixedValue", "inletOutlet"]
                    ...
                    if self.par.file_name in self.con.tables():
                        ...
                        if query.isActive():
                            ...
                            # value
                            if self.main_grid_list[bvc].itemAtPosition(0, 1).widget().currentText() == 'calculated' or \
                                self.main_grid_list[bvc].itemAtPosition(0, 1).widget().currentText() == 'nutLowReWallFunction' or \
                                self.main_grid_list[bvc].itemAtPosition(0, 1).widget().currentText() == 'fixedValue' or \
                                self.main_grid_list[bvc].itemAtPosition(0, 1).widget().currentText() == 'inletOutlet':
    ...
    def type_on_change(self, bvc):
        ...
        elif type_txt == "calculated" or type_txt == "nutLowReWallFunction" or type_txt == "fixedValue" or type_txt == "inletOutlet":
        ...
        # создаем таблицу self.par.file_name
            if self.par.file_name not in self.par.con.tables():
                ...
                for el_p in self.patches_lst:
                    ...
                    elif self.type_list_main[i].currentText() == "calculated" or self.type_list_main[i].currentText() == "nutLowReWallFunction" or self.type_list_main[i].currentText() == "fixedValue" or self.type_list_main[i].currentText() == "inletOutlet":
                        type_txt = self.main_grid_list[i].itemAtPosition(0, 1).widget().currentText()
