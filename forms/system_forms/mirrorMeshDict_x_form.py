...
class mirrorMeshDict_x_form_class(QWidget):
    ...
    def on_btnSave_clicked(self):	
        ...
        if '' not in xyz1_list and '' not in xyz2_list and planeTolerance_txt != '':
            if 'mirrorMeshDict_x' not in self.con.tables():
                ...
                query.exec("INSERT INTO mirrorMeshDict_x(name, value) VALUES ('%s', '%s')" % ('planeType', planeType_txt))
                query.exec("INSERT INTO mirrorMeshDict_x(name, value) VALUES ('%s', '%s')" % ('point', xyz1_str))
                query.exec("INSERT INTO mirrorMeshDict_x(name, value) VALUES ('%s', '%s')" % ('point', xyz2_str))
                query.exec("INSERT INTO mirrorMeshDict_x(name, value) VALUES ('%s', '%s')" % ('planeTolerance', planeTolerance_txt))
            ...
            shutil.copyfile("./matches/Shablon/system/mirrorMeshDict.x", self.full_dir + '/system/mirrorMeshDict.x')
    ...
    def data_reprint(self):
        ...
        outf = open(self.full_dir + '/system/mirrorMeshDict.x')

        data = outf.read()

        if self.interface_lng_val == 'Russian':
            self.par.outf_lbl.setText("Файл " + "<font color='peru'>" + 'mirrorMeshDict.x' + "</font>")
        elif self.interface_lng_val == 'English':
            self.par.outf_lbl.setText("<font color='peru'>" + 'mirrorMeshDict.x' + "</font>" + " file")
